...
if ($VMname) {
    ...
    try 
    {
        ...
        #ToDo Catch Error if VM not running
        $vm = Stop-AzVM -ResourceGroupName $ResourceGroup -Name $VMname -NoWait -Force -ErrorAction SilentlyContinue

        #FUNCTION RETURN
        $status = [HttpStatusCode]::OK
        $body = "VM Stop Initiated.  Wait a few minutes then check VM Status to confirm VM is Stopped (and Deallocated)"
    }
    catch
    {
        write-host $_.Exception.Message
